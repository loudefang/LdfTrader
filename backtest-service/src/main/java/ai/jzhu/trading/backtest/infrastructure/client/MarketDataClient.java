...
public class MarketDataClient implements MarketDataPort {
    ...
    @Override
    public List<KlineResponse> getKline(String symbol, String market, String period,
                                        LocalDate startDate, LocalDate endDate) {
        try {
            List<KlineResponse> result = marketDataWebClient.get()
                    .uri(uri -> {
                        var b = uri.path("/api/market-data/kline")
                                .queryParam("symbol", symbol)
                                .queryParam("market", market)
                                .queryParam("period", period);
                        if (startDate != null) b.queryParam("startDate", startDate.format(ISO_DATE));
                        if (endDate   != null) b.queryParam("endDate",   endDate.format(ISO_DATE));
                        return b.build();
                    })
                    .retrieve()
            ...
            throw new IllegalStateException("Market data service returned " + e.getStatusCode().value(), e);
        }
    }
